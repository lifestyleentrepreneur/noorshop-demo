...
- التحقق من الهوية: قبل الاستعلام عن أي طلب أو إرجاع أو معلومة خاصة بالحساب — إن كان العميل يعرف رقم طلبه، اطلبي رقم الطلب مع البريد الإلكتروني أو رقم الهاتف. إن لم يكن يعرف رقم طلبه، اطلبي اسمه الكامل وبريده الإلكتروني. لا تستخدمي أي أداة حتى يتم التحقق من الهوية.
- إذا أبدى العميل إحباطاً أو ضيقاً، تعاطفي معه أولاً ثم ساعديه.
- التخلي عن السلة: إذا ذكر العميل السعر أو التردد في الشراء أو طلب خصماً، استدعي apply_discount فوراً واعرضي عليه الكوبون المناسب.
- تقييم الخدمة (CSAT): بعد حل مشكلة العميل بنجاح، اسأليه تقييم تجربته من واحد إلى خمسة. عندما يجيب، استدعي collect_csat بالتقييم واشكريه.
...
- Cart abandonment: if the customer mentions price, says something is expensive, or asks about discounts — immediately call apply_discount and offer a promo code.
- Identity verification: before looking up any order, return, or account-specific information — if they have an order ID, ask for it plus their email or phone. If they don't have an order ID, ask for their full name and email. Do not call any order tool until identity is confirmed.
- CSAT: after successfully resolving the customer's issue, ask them to rate the experience from 1 to 5. When they reply, call collect_csat with their score and thank them.
...
def _make_vapi_tools(server_url: str) -> list:
    ...
    tool_defs = [
        ...
        {
            "name": "collect_csat",
            "description": "Record the customer's satisfaction rating (1–5) after resolving their issue.",
            "parameters": {
                "type": "object",
                "properties": {
                    "score": {"type": "integer"},
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .metrics-bar {{
      ...
      margin-top: 2.2rem; padding: 1.1rem 1.6rem;
    ...
    .metric-label {{ font-size: 0.68rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.04em; }}
    .metric-value.csat {{ color: #fbbf24; }}
...
<body>
  ...
  <div class="metrics-bar" id="metrics-bar">
    <div class="metric">
      <span class="metric-value csat" id="m-csat">—</span>
      <span class="metric-label">Avg CSAT / 5 ⭐</span>
    </div>
    <div class="metric">
      <span class="metric-value green" id="m-conv">—</span>
      <span class="metric-label">Conversations</span>
    </div>
    <div class="metric">
      <span class="metric-value blue" id="m-ratings">—</span>
      <span class="metric-label">Ratings collected</span>
    </div>
    <div class="metric">
      <span class="metric-value red" id="m-esc">—</span>
      <span class="metric-label">Escalation rate</span>
    </div>
    <div class="metric">
      <span class="metric-value green" id="m-disc">—</span>
      <span class="metric-label">Discounts applied</span>
    </div>
  </div>
  ...
  <script>
    ...
    async function refreshMetrics() {{
      try {{
        const r = await fetch(CHAT_API + "/api/metrics");
        const d = await r.json();
        document.getElementById("m-csat").textContent    = d.avg_csat.toFixed(1);
        document.getElementById("m-conv").textContent    = d.total_conversations;
        document.getElementById("m-ratings").textContent = d.total_ratings;
        document.getElementById("m-esc").textContent     = d.escalation_rate_pct + "%";
        document.getElementById("m-disc").textContent    = d.discounts_applied;
      }} catch(e) {{ /* silent fail */ }}
    }}
    refreshMetrics();
